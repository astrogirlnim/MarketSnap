...
describe("Cloud Functions: MarketSnap", () => {
  ...
  beforeEach(() => {
    // Stub Firebase admin initialization. This is important for offline testing.
    ...
    sendEachForMulticastStub = sinon.stub(admin.messaging(), "sendEachForMulticast");
  });
  ...
  describe("sendFollowerPush", () => {
    it("should send notifications to all followers on a new snap", async () => {
      ...
      docStub.withArgs("vendor1").returns({ get: () => Promise.resolve({ exists: true, data: () => vendorData }) });
      
      const followersCollectionRef = { get: () => Promise.resolve({
          empty: false,
          docs: followersData,
          forEach: (callback: (doc: any) => void) => followersData.forEach(callback),
      })};
      collectionStub.withArgs("vendors/vendor1/followers").returns(followersCollectionRef as any);
      
      sendEachForMulticastStub.resolves({ successCount: 2, failureCount: 0 });

      // Create the test event data
      const snap = testEnv.firestore.makeDocumentSnapshot(snapData, "vendors/vendor1/snaps/snap1");
      const wrapped = testEnv.wrap(sendFollowerPush);

      // Execute the function
      await wrapped({ data: snap, params: { vendorId: "vendor1", snapId: "snap1" } });
      ...
      expect(callArgs.notification.title).to.equal("The Best Veggies has a new Snap!");
    ...
    it("should not send notifications if vendor has no followers", async () => {
        ...
        const wrapped = testEnv.wrap(sendFollowerPush);
        const snap = testEnv.firestore.makeDocumentSnapshot({}, "vendors/vendor1/snaps/snap1");
  ...
  describe("fanOutBroadcast", () => {
    it("should send a broadcast to all followers", async () => {
        ...
        const followersData = [ { id: "follower1", data: () => ({ fcmToken: "token1" }) }];
        ...
        sendEachForMulticastStub.resolves({ successCount: 1, failureCount: 0 });

        const wrapped = testEnv.wrap(fanOutBroadcast);
        const broadcast = testEnv.firestore.makeDocumentSnapshot(broadcastData, "vendors/vendor1/broadcasts/broadcast1");

        await wrapped({ data: broadcast, params: { vendorId: "vendor1", broadcastId: "broadcast1" } });

        expect(sendEachForMulticastStub.calledOnce).to.be.true;
        const callArgs = sendEachForMulticastStub.firstCall.args[0];
        expect(callArgs.tokens).to.deep.equal(["token1"]);
        expect(callArgs.notification.title).to.equal("Message from Fruit Stand");
    ...
    it("should not send a broadcast if the message is missing", async () => {
        const broadcast = testEnv.firestore.makeDocumentSnapshot({ message: "" }, "vendors/vendor1/broadcasts/broadcast1");
        const wrapped = testEnv.wrap(fanOutBroadcast);
        await wrapped({ data: broadcast, params: { vendorId: "vendor1", broadcastId: "broadcast1" } });
        expect(sendEachForMulticastStub.called).to.be.false;
    });
