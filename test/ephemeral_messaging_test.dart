  ...
  group('Ephemeral Messaging Logic Tests', () {
    ...
    group('Message TTL and Expiration', () {
      ...
      test('should filter out expired messages in conversation streams', () async {
        ...
        debugPrint(
          '✅ Conversation stream correctly filters out expired messages',
        );
        debugPrint('   - Message filtering logic verified directly');
        debugPrint('   - Active messages: ${filteredMessages.length}');
        debugPrint(
          '   - Expired messages filtered out: ${messages.length - filteredMessages.length}',
        );
      });
    });
    ...
    group('TTL Cleanup Functionality', () {
      test(
        ...
        () async {
          ...
          final remainingMessage = Message.fromFirestore(
            afterCleanup.docs.first,
          );
          expect(remainingMessage.text, equals('Active message'));
          expect(remainingMessage.hasExpired, isFalse);

          debugPrint(
            '✅ Manual cleanup successfully removed ${expiredMessages.length} expired messages',
          );
        },
      );
      ...
      test('should handle cleanup when no expired messages exist', () async {
        ...
        debugPrint(
          '✅ Cleanup gracefully handles collections with no expired messages',
        );
      });
    });
    ...
    group('Conversation Auto-Deletion (24h Test)', () {
      test(
        ...
        () async {
          ...
          final conversationMessages = [
            {
              ...
              'expiresAt': Timestamp.fromDate(
                pastTime.add(const Duration(hours: 24)),
              ),
              'isRead': true,
            },
            {
              ...
              'participants': [toUid, fromUid]..sort(),
              'createdAt': Timestamp.fromDate(
                pastTime.add(const Duration(minutes: 5)),
              ),
              'expiresAt': Timestamp.fromDate(
                pastTime.add(const Duration(hours: 24, minutes: 5)),
              ),
              'isRead': true,
            },
            {
              ...
              'participants': [toUid, fromUid]..sort(),
              'createdAt': Timestamp.fromDate(
                pastTime.add(const Duration(minutes: 10)),
              ),
              'expiresAt': Timestamp.fromDate(
                pastTime.add(const Duration(hours: 24, minutes: 10)),
              ),
              'isRead': true,
            },
          ];
          ...
          await expectLater(
            ...
            emits(
              predicate<List<Message>>((conversations) {
          ...
          debugPrint(
            '✅ Conversation auto-deletion after 24h successfully verified',
          );
      ...
      test(
        ...
        () async {
          ...
          debugPrint(
            '✅ Mixed conversation cleanup preserves active conversations',
          );
        },
      );
    });
