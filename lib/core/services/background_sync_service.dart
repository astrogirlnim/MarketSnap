...
void callbackDispatcher() {
  ...
  Workmanager().executeTask((task, inputData) async {
    ...
    try {
      // Check if this is our sync task
      if (task == syncTaskName) {
        debugPrint("Executing background sync task for pending media...");
        ...
        debugPrint("Unknown task received: $task");
        return Future.value(false);
      }
    } catch (err) {
      debugPrint("An error occurred during background sync: $err");
...
class BackgroundSyncService {
  ...
  Future<void> scheduleOneTimeSyncTask() async {
    debugPrint("Scheduling a one-time background sync task...");
