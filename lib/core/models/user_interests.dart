...
class UserInterests {
  ...
  categoryRelevanceScores; // category -> avg relevance
  ...
  factory UserInterests.fromFirestore(
    ...
    return UserInterests(
      ...
      keywordInteractions: Map<String, int>.from(
        data['keywordInteractions'] ?? {},
      ),
      categoryInteractions: Map<String, int>.from(
        data['categoryInteractions'] ?? {},
      ),
      keywordRelevanceScores: Map<String, double>.from(
        data['keywordRelevanceScores'] ?? {},
      ),
      categoryRelevanceScores: Map<String, double>.from(
        data['categoryRelevanceScores'] ?? {},
      ),
  ...
  UserInterests updateWithFeedback({
    ...
    final newKeywordRelevanceScores = Map<String, double>.from(
      keywordRelevanceScores,
    );

    for (final keyword in keywords) {
      newKeywordInteractions[keyword] =
          (newKeywordInteractions[keyword] ?? 0) + 1;
    ...
    final newCategoryRelevanceScores = Map<String, double>.from(
      categoryRelevanceScores,
    );
    ...
    final currentCategoryScore =
        newCategoryRelevanceScores[category] ?? relevanceScore;
    final currentCategoryCount = newCategoryInteractions[category]!;
    newCategoryRelevanceScores[category] =
        ((currentCategoryScore * (currentCategoryCount - 1)) + relevanceScore) /
    ...
    return UserInterests(
      ...
      preferredContentType:
          preferredContentType, // Could be updated based on patterns
  ...
  bool get hasSignificantData =>
      totalInteractions >= 5 && preferredKeywords.isNotEmpty;
  ...
  double get personalizationConfidence {
    ...
    final interactionScore = (totalInteractions / 50.0).clamp(
      0.0,
      1.0,
    ); // Max at 50 interactions
    final satisfactionWeight = satisfactionScore;
    final engagementWeight = engagementRate;

    return (interactionScore * 0.4 +
