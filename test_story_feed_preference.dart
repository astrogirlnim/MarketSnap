...
import 'package:marketsnap/features/settings/application/settings_service.dart';
...
void main() {
  group('UserSettings Posting Preference Tests', () {
    test('should default to feed posting preference (false)', () {
      ...
      expect(settings.preferStoryPosting, false, 
    ...
    test('should create UserSettings with all fields including posting preference', () {
      final settings = UserSettings(
    ...
    test('should include posting preference in toString output', () {
      ...
      expect(stringOutput.contains('preferStoryPosting: true'), true,
    ...
    test('should maintain separate values for different preferences', () {
      ...
      expect(feedSettings.preferStoryPosting != storySettings.preferStoryPosting, true,
          reason: 'Different instances should maintain separate preference values');
    });
  });

  group('UserSettings Integration Tests', () {
    test('should maintain default values for other fields when setting posting preference', () {
      final settings = UserSettings(preferStoryPosting: true);
      
      // Verify other fields maintain their defaults
      expect(settings.enableCoarseLocation, false, reason: 'Default should be maintained');
      expect(settings.autoCompressVideo, true, reason: 'Default should be maintained');
      expect(settings.saveToDeviceDefault, false, reason: 'Default should be maintained');
      expect(settings.preferStoryPosting, true, reason: 'Specified value should be set');
    });
  });
  ...
  group('Story vs Feed Logic Simulation', () {
    test('should simulate MediaReviewScreen posting choice logic', () {
      ...
      var nextSessionSettings = UserSettings(
        ...
        autoCompressVideo: userSettings.autoCompressVideo,
        saveToDeviceDefault: userSettings.saveToDeviceDefault,
        preferStoryPosting: userSettings.preferStoryPosting, // Persisted from previous session
      );
      var nextSessionPostToStory = nextSessionSettings.preferStoryPosting;
      expect(nextSessionPostToStory, true, 
